...
- `channel_freqs`     : Absolute channel center frequencies (Hz).
- `ch_widths`         : Per-channel width (Hz).
- `total_bandwidths`  : Per-channel total bandwidth (Hz).
- `sidebands`         : Per-channel sideband (+1 = upper, -1 = lower).
...
function Base.show(io::IO, fs::FrequencySetup)
    ...
    return print(io, "FrequencySetup($(fs.name), $(length(fs.channel_freqs)) channels, $(flo)–$(fhi) GHz)")
end
